...
class GuestBook:
    ...
    def make_json(filename = 'data.json'):
        with open(filename, 'w') as file:
            file.write('')

    
    def delete_json(filename = 'data.json'):
        path = os.path.join(os.path.abspath(os.path.dirname(__file__)), filename)
        os.remove(path)


    def rename_json(new_filename, filename = 'data.json'):
    ...
    def write_json(json_text, filename = 'data.json'):
        with open(filename, 'w') as file:
